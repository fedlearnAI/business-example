...
import com.jdt.fedlearn.frontend.entity.match.MatchProgressReq;
...
public class ForwardController {
     ...
     * @return ResponseEntity<Map>
     */
    @RequestMapping(value = "prepare/parameter/algorithm", method = RequestMethod.POST, produces = RequestConstant.HEADER)
    @ResponseBody
    ...
    public ResponseEntity<ModelMap> matchProgress(@Validated @RequestBody MatchProgressReq matchProgressReq) {
        ...
        request.put("matchId", matchProgressReq.getMatchId());
     ...
     * TODO 后续会优化前端，
     */
    @RequestMapping(value = RequestConstant.MATCH_LIST, method = RequestMethod.POST, produces = RequestConstant.HEADER)
    @ResponseBody
    public ResponseEntity<ModelMap> matchList(@Validated @RequestBody Map<String, Object> request) {
        request.remove(USER_NAME);
...
//        //TODO
//        request.put("matchId", queryMatchList(taskId).get(0));
...
//    }
    ...
    public ResponseEntity<ModelMap> predict(@Validated @RequestBody ModelMap request) {
        String taskId = request.get("model").toString().split("-")[0];
        ...
        request.put(CLIENT_LIST, partnerInfos);
        request.remove(USER_NAME);
        boolean use = (boolean) request.get("use");
        request.put("secureMode", use);
        String model = (String) request.get("model");
        request.put("modelToken", model);
        request.remove("use");
