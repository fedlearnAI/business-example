...
import com.jdt.fedlearn.frontend.util.TimeUtil;
 ...
 * 用户权限相关
 * @author geyan
 * @version 0.9.1 2021/3/9 4:17 下午
 */
@Controller
@RequestMapping("api")
public class AccountController {
    ...
    */
    ...
    @ResponseBody
    public ResponseEntity<ModelMap> login(HttpServletRequest httpServletRequest,@Validated @RequestBody AccountDO accountDO) {
        ModelMap result;
        String verifyCodePic = "1";//accountDO.getVerifyCodePic();
        String captchaId = "1";//(String) httpServletRequest.getSession().getAttribute("verifyCode");
        if(verifyCodePic!=null && !"".equals(verifyCodePic) &&verifyCodePic.toUpperCase().equals(captchaId.toUpperCase())){
            String userName = accountDO.getUsername();
            AccountDO oldAccountDO = accountService.queryAccount(userName);
            ...
            result = ResponseHandler.failResponse("图片验证码错误！");
        }
    ...
    @ResponseBody
    public ResponseEntity<ModelMap> queryUserList(HttpServletRequest httpServletRequest,@Validated @RequestBody AccountDO accountDO) {
